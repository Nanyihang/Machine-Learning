...
bestAcc = -1
bestModel = None
learn_rates = [0.01e-7, 0.1e-7, 1e-7, 10e-7]
reg_strengths = [10,100,1000,10000]
...
for reg_strength in reg_strengths:
    for learn_rate in learn_rates:
        classifier = Svm(xTrain.shape[1],numClasses)
        classifier.train(xTrain,yTrain,learn_rate,reg_strength,iter=1500,verbose=False)
        Acc = classifier.calAccuracy(xVal,yVal)
        if Acc > bestAcc:
            bestAcc = Acc
            bestModel = classifier
        bestParameters = [Acc,reg_strength]
################################################################################
#                              END OF YOUR CODE                                #
################################################################################
