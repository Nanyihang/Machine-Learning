...
class Softmax (object):
    ...
    def calLoss (self, x, y, reg):
        ...
        N = x.shape[0]
        Score = x.dot(self.W)
        Score_max = Score-np.max(Score,axis= 1, keepdims=True)
        exp_Score_max = np.exp(Score_max)
        sum_all = np.sum(exp_Score_max,axis=1,keepdims=True)
        probability = exp_Score_max /sum_all
        probability_yi = probability[np.arange(N),y]
        loss_i = -np.log(probability_yi)
        loss = np.sum(loss_i)/N
        loss += reg * np.sum(self.W * self.W)
        d_score = probability.copy()
