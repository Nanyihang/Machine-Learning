...
# Select device
deviceType = "/cpu:0"
tfConfig = tf.ConfigProto(intra_op_parallelism_threads=20, inter_op_parallelism_threads=15, allow_soft_placement=True, device_count = {'CPU': 10})
...
def complexModel():
    with tf.device(deviceType):
        ...
        # Define Loss
        totalLoss = tf.losses.hinge_loss(tf.one_hot(y, 10), logits=yOut)
        meanLoss = tf.reduce_mean(totalLoss) + 5e-4*tf.nn.l2_loss(wConv) + 5e-4*tf.nn.l2_loss(W1) + 5e-4*tf.nn.l2_loss(W2)
